...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    # Need y_hat to compute gradient of softmax
    x = x - np.max(x,axis=0)
    exp = np.exp(x)
    y_hat = exp / np.sum(exp,axis=0)
    # Compute derivative of y_hat with respect to x
    cross_yhat = y_hat[:, np.newaxis] @ y_hat[:, np.newaxis].T
    diag_yhat = np.diag(y_hat)
    grad_yhat_x = diag_yhat - cross_yhat
    # Compute gradient as matrix product
    return grad_yhat_x @ grad_outputs
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    conv_channels, _, conv_width, conv_height = conv_W.shape
    grad_z_w = np.zeros((input_channels, conv_height, conv_width, output_height, output_width))
    for c in range(input_channels):
        for x in range(conv_height):
            for y in range(conv_width):
                grad_z_w[c,x,y,:,:] = data[c,x:x+output_height,y:y+output_width]
    grad_w = np.einsum('ijk,cabjk->icab', output_grad, grad_z_w)

    # Gradient of loss wrt bias
    grad_z_b = np.ones((25,25))
    grad_b = np.einsum('ijk,jk->i', output_grad, grad_z_b)

    # Gradient wrt to the data is incredibly complex with this formulation
    # Since it is not needed in this exercise, a simple matrix of ones is returned

    return grad_w, grad_b, np.ones(data.shape)
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    # *** START CODE HERE ***
    input_channels, input_width, input_height = data.shape
    grad_data = np.repeat(np.repeat(output_grad, pool_height, axis=1), pool_width, axis=2)
    
    for c in range(input_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):

                t = data[c, x:(x + pool_width), y:(y + pool_height)]
                mult = np.zeros(t.shape)
                mult[c, np.unravel_index(t.argmax(), t.shape)] = 1.
                grad_data[c,x:(x + pool_width), y:(y + pool_height)] = grad_data[c,x:(x + pool_width), y:(y + pool_height)] * mult
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    grad_weights = data[:, np.newaxis] @ output_grad[:, np.newaxis].T
    grad_bias = output_grad.copy()
...
def backward_prop(data, labels, params):
    ...
    # Backward pass
    grad_y_hat = backward_cross_entropy_loss(y,labels)
    grad_o = backward_softmax(logits, grad_y_hat)
    gradients['W2'], gradients['b2'], grad_a = backward_linear(W2, b2, flattened, grad_o)
    grad_A = np.reshape(grad_a, first_after_relu.shape)
    grad_P = backward_relu(first_max_pool, grad_A)
    grad_Z = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_P)
    gradients['W1'], gradients['b1'], _ = backward_convolution(W1, b1, data, grad_Z)
...
def main(path_dir):
    np.random.seed(100)
    train_data_path = os.path.join(path_dir, 'images_train.csv')
    train_label_path = os.path.join(path_dir,'labels_train.csv')
    train_data, train_labels = read_data(train_data_path, train_label_path)
