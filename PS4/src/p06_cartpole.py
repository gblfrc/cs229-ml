...
def choose_action(state, mdp_data):
    ...
    # Select the probabilities for the current state
    P_sa = mdp_data['transition_probs'][state].T
    # Compute sum of products P_sa(s')*V(s') over the states s' 
    prodsums = P_sa @ mdp_data['value']
    # The optimal action is the one that returns the maximum sum of the products
    # P_sa(s') V(s') on all states s'.
    return np.argmax(prodsums)
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # Number of times the `new_state` is reached
    mdp_data['reward_counts'][new_state, 1] += 1
    # Number of times reward -1 is observed
    if reward == -1:
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # Create a copy of the probabilities for convenience
    init_probs = mdp_data['transition_probs'].copy()

    # Recompute probabilities for each action
    for a in range(init_probs.shape[-1]):

        # Get probabilities and counts for current action
        p_a = init_probs[:,:,a]
        c_a = mdp_data['transition_counts'][:,:,a].copy()
        # Compute new probabilities
        c_sums = np.repeat(np.sum(c_a, axis=1), c_a.shape[0]).reshape(c_a.shape) # Total counts for each s-a pair
        p_a[c_sums[:,:] != 0] = (c_a[c_sums[:,:] != 0]/c_sums[c_sums[:,:] != 0])  # New probabilities based on counts
        # Update probability matrix
        mdp_data['transition_probs'][:,:,a] = p_a

    # Update rewards
    r_c = mdp_data['reward_counts']
    mdp_data['reward'][r_c[:,1] != 0] = -r_c[r_c[:,1] != 0, 0] / r_c[r_c[:,1] != 0, 1]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        # Read parameters into variables for convenience
        P_sa = mdp_data['transition_probs']
        V_s = mdp_data['value']
        # Compute second term of the Bellman equation
        B_sa = np.einsum("ija,j->ia", P_sa, V_s)
        B_sa_max = np.max(gamma*B_sa, axis=1)
        # Update V_s by computing full Bellman equation
        V_s_new = mdp_data['reward'] + B_sa_max
        mdp_data['value'] = V_s_new

        # Check termination
        if np.max(np.abs(V_s_new - V_s)) < tolerance:
            break
